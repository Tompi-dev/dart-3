...
class StudentsHandler {
  Router get router {
    ...
    router.post('/groups/<id>/join', (Request req, String id) async {
      ...
      if (user == null || user['role'] != 'student') {
        return Response.forbidden(
          jsonEncode({
            'error': 'Access denied. Only students can join groups.',
          }),
          headers: {'content-type': 'application/json'},
        );
      }
      ...
      '''),
        ...
        if (conflictCount > 0) {
          return Response(
            ...
            body: jsonEncode({
              'error':
                  'Schedule conflict: student already has a class at this time.',
            }),
            headers: {'content-type': 'application/json'},
          );
        }
        ...
        await connection.execute(
          Sql.named('''
        INSERT INTO group_students (group_id, student_id, is_trial, created_by, created_at)
        VALUES (@groupId, @studentId, FALSE, @studentId,  NOW())
      '''),
    ...
    router.post('/groups/<id|[0-9]+>/join-trial', (
      Request req,
      String id,
    ) async {
      final user = req.context['user'] as Map?;
      if (user == null || user['role'] != 'student') {
        return Response.forbidden(
          jsonEncode({
            'error': 'Access denied. Only students can join trials.',
          }),
          headers: {'content-type': 'application/json'},
        );
      }
      ...
      '''),
        ...
        final exists = await connection.execute(
          Sql.named(
            'SELECT COUNT(*) FROM group_students WHERE student_id = @studentId AND group_id = @groupId',
          ),
          parameters: {'studentId': studentId, 'groupId': groupId},
        );
 ...
 OVERLAPS (target.start_time, target.start_time + INTERVAL '90 minutes'))
      '''),
        ...
        await connection.execute(
          Sql.named(
            'INSERT INTO group_students (group_id, student_id, is_trial,  created_at) VALUES (@groupId, @studentId, TRUE,  NOW())',
          ),
          parameters: {'groupId': groupId, 'studentId': studentId},
        );
