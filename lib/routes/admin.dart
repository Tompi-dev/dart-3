...
class AdminHandler {
  Router get router {
    ...
    r.get('/admin/analytics', _analytics);
      ...
      try {
        final rows = await connection.execute(
          Sql.named('''
            SELECT h.id AS hall_id, h.name AS hall_name,
                   g.id AS group_id, g.start_time AS start_time, g.duration_min AS duration_min
            FROM halls h
            LEFT JOIN groups g
              ON g.hall_id = h.id
              AND g.cancelled = FALSE
              AND tstzrange(g.start_time, g.start_time + (g.duration_min || ' minutes')::interval) &&
        ...
        final list = rows.map((r) {
          return {
            ...
            'busy_duration_min': r[4],
          };
        }).toList();

        return Response.ok(jsonEncode({'busy': list}), headers: {'content-type': 'application/json'});
      } catch (e) {
    ...
    r.post('/exceptions/approve', (Request req) async {
      try {
        ...
        if (!approve) {
          ...
          await connection.execute(
            Sql.named("INSERT INTO logs (action, details) VALUES ('exception_review', @d)"),
            parameters: {'d': jsonEncode({'exception_id': exceptionId, 'approved': false})},
          );
          return Response.ok(jsonEncode({'ok': true, 'approved': false}),
              headers: {'content-type': 'application/json'});
        }

        final exRows = await connection.execute(
          Sql.named('SELECT id, group_id, exception_type, new_time FROM schedule_exceptions WHERE id = @id'),
          parameters: {'id': exceptionId},
        );

        if (exRows.isEmpty) {
          return Response.notFound(jsonEncode({'error': 'Exception not found'}), headers: {'content-type': 'application/json'});
        }
        ...
        await connection.execute(
          Sql.named('UPDATE schedule_exceptions SET approved_by_admin = TRUE WHERE id = @id'),
          parameters: {'id': exceptionId},
        );

        await connection.execute(
          Sql.named("INSERT INTO logs (action, details) VALUES ('exception_approved', @d)"),
          parameters: {'d': jsonEncode({'exception_id': exceptionId, 'approved': true})},
        );

        return Response.ok(jsonEncode({'ok': true, 'approved': true}), headers: {'content-type': 'application/json'});
      } catch (e) {
        return Response.internalServerError(body: jsonEncode({'error': e.toString()}), headers: {'content-type': 'application/json'});
      }
    });
    ...
    r.post('/teachers/<id|[0-9]+>/limit', (Request req, String id) async {
      try {
        ...
        if (exists.isEmpty) {
          return Response.notFound(jsonEncode({'error': 'Teacher not found'}), headers: {'content-type': 'application/json'});
        }
        ...
        await connection.execute(
          Sql.named("INSERT INTO logs (action, details) VALUES ('teacher_limit', @d)"),
          parameters: {'d': jsonEncode({'teacher_id': teacherId, 'is_frozen': true})},
        );

        return Response.ok(jsonEncode({'ok': true, 'teacher_id': teacherId, 'is_frozen': true}),
            headers: {'content-type': 'application/json'});
      } catch (e) {
        return Response.internalServerError(body: jsonEncode({'error': e.toString()}), headers: {'content-type': 'application/json'});
      }
    ...
    r.post('/groups/<id|[0-9]+>/add-student', (Request req, String id) async {
      try {
        ...
        if (studentId == null) {
          return Response(400, body: jsonEncode({'error': 'student_id required'}), headers: {'content-type': 'application/json'});
        }
        ...
        await connection.execute(
          Sql.named('''
            ...
            SELECT @id, ('student_' || @id::text || '@local'), ('Student ' || @id::text), '', 'student'
        ...
        await connection.execute(
          Sql.named('INSERT INTO students (id) VALUES (@id) ON CONFLICT (id) DO NOTHING'),
          parameters: {'id': studentId},
        );

        await connection.execute(
          Sql.named('INSERT INTO group_students (group_id, student_id, is_trial) VALUES (@gid, @sid, @trial) ON CONFLICT DO NOTHING'),
          parameters: {'gid': gid, 'sid': studentId, 'trial': isTrial},
        );

        await connection.execute(
          Sql.named("INSERT INTO logs (action, details) VALUES ('admin_add_student', @d)"),
          parameters: {'d': jsonEncode({'group_id': gid, 'student_id': studentId, 'is_trial': isTrial})},
        );
  ...
  // handler function reused for both routes
  Future<Response> _analytics(Request req) async {
    try {
      ...
      final teachersRows = await connection.execute(
        Sql.named('''
          SELECT u.full_name, COUNT(*)::int AS classes
          FROM groups g
      ...
      return Response.ok(jsonEncode({'by_hall': byHall, 'by_teacher': byTeacher}), headers: {'content-type': 'application/json'});
    } catch (e) {
      return Response.internalServerError(body: jsonEncode({'error': e.toString()}), headers: {'content-type': 'application/json'});
    }
  }
